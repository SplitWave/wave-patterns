...
export async function fetchMarginfiData(walletAddress: string) {
  // ----------------------
  // Setup client
  // ----------------------
  ...
  return { banksShaped, userAccountsShaped };
}
...
function shapeBank(bank: Bank, bankMetadata: BankMetadata): BankSnapshot {
  return {
    ...
    totalBorrowsUsdValue: bank
      .getLiabilityUsdValue(
        bank.totalLiabilities,
        MarginRequirementType.Equity,
        PriceBias.None
      )
      .toNumber(),
